...
function ThreadDetail() {
              ...
              <p className="flex flex-col gap-4">
                ...
                <span>
                  {error.status_code === 404
                    ? "The thread you are looking for does not exist. You can browse other threads or create a new one."
                    : "This thread may have been deleted or does not exist. Please try again later."}
                </span>
              </p>
            </AlertDialogDescription>
          </AlertDialogHeader>
  ...
  const isUpvoted = thread?.data?.user_vote === VoteType.UPVOTE;
  const isDownvoted = thread?.data?.user_vote === VoteType.DOWNVOTE;
  ...
  const handleDelete = async () => {
    try {
      toast.promise(deleteThread(threadId ?? ""), {
            ...
            <main className="prose prose-sm max-w-none flex flex-col gap-4">
              ...
              <section className="self-end">
                <VoteButtons
                  ...
                  handleUpVote={() =>
                    handleVote(thread?.data?.id || "", VoteType.UPVOTE)
                  }
                  handleDownVote={() =>
                    handleVote(thread?.data?.id || "", VoteType.DOWNVOTE)
                  }
                  upVoted={isUpvoted}
