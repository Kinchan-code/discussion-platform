 ...
 * @param {string | null} [params.highlightComment] - Optional comment ID to highlight.
 * @param {string | null} [params.highlightReply] - Optional reply ID to highlight.
    ...
    const response = await api.get<Response<Comments[]>>(
      `/threads/${params.threadId}/comments`,
      {
        params: {
          ...
          highlight_comment: params.highlightComment,
          highlight_reply: params.highlightReply,
          per_page: 5,
        },
      }
    );
...
export const useGetComments = ({ params }: QueryParams) => {
  return useQuery<Response<Comments[]>, ResponseError>({
    queryKey: [
      ...
      params.threadId,
      params.author,
      params.highlightComment,
      params.highlightReply,
    ],
